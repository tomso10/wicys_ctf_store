...
func (uq *UserQuery) loadTransactions(ctx context.Context, query *TransactionQuery, nodes []*User, init func(*User), assign func(*User, *Transaction)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_transactions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) loadTokens(ctx context.Context, query *TokenQuery, nodes []*User, init func(*User), assign func(*User, *Token)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.TokensTable)
		s.Join(joinT).On(s.C(token.FieldID), joinT.C(user.TokensPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.TokensPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.TokensPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*Token](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
