...
func (tq *TokenQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Token, error) {
	...
	if query := tq.withUser; query != nil {
		if err := tq.loadUser(ctx, query, nodes,
			func(n *Token) { n.Edges.User = []*User{} },
			func(n *Token, e *User) { n.Edges.User = append(n.Edges.User, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TokenQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Token, init func(*Token), assign func(*Token, *User)) error {
	...
	byID := make(map[int]*Token)
	nids := make(map[int]map[*Token]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(token.UserTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(token.UserPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(token.UserPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(token.UserPrimaryKey[0]))
