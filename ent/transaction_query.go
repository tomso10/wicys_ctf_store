...
func (tq *TransactionQuery) QueryItem() *ItemQuery {
...
func (tq *TransactionQuery) WithItem(opts ...func(*ItemQuery)) *TransactionQuery {
	query := (&ItemClient{config: tq.config}).Query()
...
func (tq *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	...
	if tq.withItem != nil || tq.withUser != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := tq.withItem; query != nil {
		if err := tq.loadItem(ctx, query, nodes, nil,
			func(n *Transaction, e *Item) { n.Edges.Item = e }); err != nil {
			return nil, err
		}
	}
...
func (tq *TransactionQuery) loadItem(ctx context.Context, query *ItemQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Item)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		if nodes[i].transaction_item == nil {
			continue
		}
		fk := *nodes[i].transaction_item
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "transaction_item" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
